...
class ShLowStockDetailsReportWizard(models.TransientModel):
    ...
    def print_low_stock_xls_report(self):
        ...
        if Products:
            ...
            if self.env.company.product_quantity_check != 'global':
                worksheet.write_merge(5, 6, 4, 4, "Minimum Qty Stock", bold)
            ...
            for rec in journal_lines:
                ...
                if rec.get('name'):
                    worksheet.write(row, 1, str(rec.get('name')))
                if rec.get('def_code'):
                    worksheet.write(row, 2, str(rec.get('def_code')))
                if rec.get('prod_qty'):
                    worksheet.write(row, 3, str(rec.get('prod_qty')))
                if rec.get('min_qty'):
                    worksheet.write(row, 4, str(rec.get('min_qty')))

                row += 1

            filename = ('Low Stock Notification Detail Xls Report' + '.xls')
            fp = BytesIO()
            workbook.save(fp)
