...
class CustomerRegisterPayment(models.Model):
    _name = "kb.customer.register.payment"
    ...
    kb_CustomerName = fields.Many2one('res.partner', string='Customer')
    ...
    kb_companyBankAccount = fields.Many2one('res.partner.bank' , string='Company Bank Account')
    ...
    def action_confirm_payment(self):
        if self.kb_payment_type == 'inbound':
            #Recivable
            self.move_id.update({
                'date': self.kb_date,
                'journal_id': self.journal_id,
                'line_ids': [(0, 0, {
                    'partner_id': self.kb_CustomerName.id,
                    'name': 'Customer Payment %s, %s , %s' % (self.kb_amount, self.kb_CustomerName.name, self.kb_date),
        ...
        for rec in self:
            ...
            if rec.approval_config_id and rec.approval_config_id.payment_approval_line:
                ...
                for user in users:
                    ...
                    super(CustomerRegisterPayment, rec)
                    
        for rex in rec.user_ids:
            activ_id = self.env['mail.activity'].create({
                'activity_type_id': 4,
                'date_deadline': datetime.today(),
                'summary': "You Have Payment Approval",
                'user_id': rex.id,
                'res_model_id': 750,
                'res_id': self.id,
    ...
    def create(self, vals):
        ...
        res = super(CustomerRegisterPayment, self).create(vals)
        return res
    ...
    approval_info_line = fields.One2many(
        'kb.approval.info', 'sh_payment_id')
...
class AccountMove(models.Model):
    ...
    int_payment_ids = fields.One2many('kb.customer.register.payment', 'move_id', string='Internal payment')
