...
QUnit.test('basic rendering of message', async function (assert) {
    ...
    assert.strictEqual(
        message.querySelectorAll(`:scope .o_Message_content_wp`).length,
        1,
        "should have content in core part of message"
    );
    assert.strictEqual(
        message.querySelector(`:scope .o_Message_content_wp`).textContent.trim(),
        "body",
...
QUnit.test('basic rendering of squashed message', async function (assert) {
    ...
    assert.strictEqual(
        message2.querySelectorAll(`:scope .o_Message_content_wp`).length,
        1,
        "message 2 should have some content in core part"
    );
    assert.strictEqual(
        message2.querySelector(`:scope .o_Message_content_wp`).textContent.trim(),
        "body2",
...
QUnit.test('message origin redirect to channel', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelectorAll('.o_Discuss_thread .o_Message_wp').length,
        2,
        "should have 2 messages"
    );
    assert.strictEqual(
        document.querySelectorAll(`
            .o_Discuss_thread
            .o_Message_wp[data-message-local-id="${
                this.env.models['mail.message'].findFromIdentifyingData({ id: 100 }).localId
            }"]
        `).length,
        1,
...
QUnit.test('inbox: mark all messages as read', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelectorAll(`.o_Discuss .o_Message_wp`).length,
        2,
...
QUnit.test('toggle_star message', async function (assert) {
    ...
    let message = document.querySelector(`.o_Discuss .o_Message_wp`);
    assert.notOk(
    ...
    message = document.querySelector(`.o_Discuss .o_Message_wp`);
    assert.ok(
        message.classList.contains('o-starred'),
...
QUnit.test('post a simple message', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelectorAll(`.o_Message_wp`).length,
        0,
    ...
    const message = document.querySelector(`.o_Message_wp`);
    assert.strictEqual(
    ...
    assert.strictEqual(
        message.querySelector(`:scope .o_Message_content_wp`).textContent,
        "Test",
...
QUnit.test('receive new needaction messages', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelector(`.o_Discuss_thread .o_Message_wp`).dataset.messageLocalId,
        this.env.models['mail.message'].findFromIdentifyingData({ id: 100 }).localId,
...
QUnit.test('reply to message from inbox (message linked to document)', async function (assert) {
    ...
    assert.strictEqual(
        document.querySelector('.o_Message_wp').dataset.messageLocalId,
        this.env.models['mail.message'].findFromIdentifyingData({ id: 100 }).localId,
    ...
    assert.ok(
        document.querySelector('.o_Message_wp').classList.contains('o-selected'),
