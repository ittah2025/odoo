    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                SELECT
                    ...
                    m.id AS move_id,
                    m.name AS move_name,
                    m.kb_description AS description,
        ...
        for partner in partner_ids:
            ...
            sql = ('''
                SELECT
                    ...
                    (select "kb_invNo" from purchase_order where name = m.invoice_origin) AS invno
                FROM account_move_line l
                JOIN account_move m ON (l.move_id=m.id)
                JOIN account_account a ON (l.account_id=a.id)
                --LEFT JOIN account_account_type AS ty ON a.user_type_id = ty.id
    ...
    def action_xlsx(self):
        ...
        if filter.get('include_details', False):
            ...
            sheet.write(row_pos, 3, _('Move'),
                                    format_header)
            sheet.write(row_pos, 4, _('Description'),
                                    format_header)
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            ...
                            sheet.write(row_pos, 1, sub_line.get('lcode'),
                                                    line_header_light)
                            sheet.write(row_pos, 2, sub_line.get('account_name').get('ar_001') or '',
                                                     line_header_light)
                            # sheet.write(row_pos, 3, sub_line.get('lref') or '',
                            #                         line_header_light)
                            sheet.write(row_pos, 3, sub_line.get('move_name'),
                                                    line_header_light)
                            sheet.write(row_pos, 4, sub_line.get('description'),
                                                    line_header_light)
