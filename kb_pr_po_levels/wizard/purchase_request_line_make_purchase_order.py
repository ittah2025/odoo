...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _prepare_item(self, line):
        return {
            "line_id": line.id,
            "request_id": line.kb_product_line_ids.id,
            "product_id": line.kb_product_id.id,
            "name": line.kb_product_name,
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env["kb_pr_level_line"].browse(request_line_ids):
            if line.kb_product_line_ids.state == "done":
                raise UserError(_("The purchase has already been completed."))
            if line.kb_product_line_ids.state not in ["approved", "in_progress"]:
                raise UserError(
                    _("Purchase Request %s is not approved or in progress")
                    % line.kb_product_line_ids.name
                )
            ...
            line_picking_type = line.kb_product_line_ids.picking_type_id or False
    ...
    def get_items(self, request_line_ids):
        request_line_obj = self.env["kb_pr_level_line"]
        items = []
    ...
    def default_get(self, fields):
        ...
        request_line_ids = []
        if active_model == "kb_pr_level_line":
            request_line_ids += self.env.context.get("active_ids", [])
        elif active_model == "kb_pr_level":
            request_ids = self.env.context.get("active_ids", False)
            request_line_ids += (
                self.env[active_model].browse(request_ids).mapped("kb_product_line_ids.id")
            )
        if not request_line_ids:
            return res
        res["item_ids"] = self.get_items(request_line_ids)
        request_lines = self.env["kb_pr_level_line"].browse(request_line_ids)
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        return {
            ...
            "name":item.line_id.kb_product_name,
            "product_uom": product.uom_po_id.id or product.uom_id.id,
            "price_unit": 0.0,
            "product_qty": qty,
            # "analytic_distribution": item.line_id.analytic_account_id,
    ...
    def make_purchase_order(self):
        ...
        pr_line_obj = self.env["kb_pr_level_line"]
        purchase = False
        ...
        for item in self.item_ids:
            ...
            if not purchase:
                po_data = self._prepare_purchase_order(
                    line.kb_product_line_ids.picking_type_id,
                    # line.request_id.group_id,
                    line.company_id,
                    line.kb_product_line_ids.note,
                )
...
class PurchaseRequestLineMakePurchaseOrderItem(models.TransientModel):
    ...
    line_id = fields.Many2one(
        comodel_name="kb_pr_level_line", string="Purchase Request Line"
    )
    request_id = fields.Many2one(
        comodel_name="kb_pr_level",
        related="line_id.kb_product_line_ids",
