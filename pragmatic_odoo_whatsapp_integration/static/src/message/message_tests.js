...
QUnit.test('basic rendering', async function (assert) {
    ...
    const messageEl = document.querySelector('.o_Message_wp');
    assert.strictEqual(
    ...
    assert.strictEqual(
        messageEl.querySelectorAll(`:scope .o_Message_content_wp`).length,
        1,
...
QUnit.test('subtype description should be displayed if it is different than body', async function (assert) {
    ...
    assert.containsOnce(
        document.body,
        '.o_Message_content_wp',
        "message should have content"
    );
    assert.strictEqual(
        document.querySelector(`.o_Message_content_wp`).textContent,
        "HelloBonjour",
...
QUnit.test('data-oe-id & data-oe-model link redirection on click', async function (assert) {
    ...
    assert.containsOnce(
        document.querySelector('.o_Message_content_wp'),
        'a',
        "message content should have a link"
    );

    document.querySelector(`.o_Message_content_wp a`).click();
    assert.verifySteps(
...
QUnit.test('message should not be considered as "clicked" after clicking on its author name', async function (assert) {
    ...
    await nextAnimationFrame();
    assert.doesNotHaveClass(
        document.querySelector(`.o_Message_wp`),
        'o-clicked',
